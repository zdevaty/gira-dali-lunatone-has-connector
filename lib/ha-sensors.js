...
export function createHaSensors({
  ...
  gatewayHost = null,
  entityFor = () => null,
  // More entities, from elsewhere: driver diagnostics and DALI-2 sensors.
  // Each is a function returning [{ entity_id, state, attributes }].
  extras = [],
  prefix = 'dali_bridge',
  heartbeatMs = 60_000,
  minGapMs = 5_000,
  ...
  function noteEvent(event) {
    ...
    switch (event.kind) {
      ...
      case 'connection':
        ...
        break;
      case 'alert':
        // Our own scan's or identify's traffic, not a fault: an automation must
        // not fire on it.
        if (event.during_scan || event.during) break;
        alerts += 1;
  ...
  function states({ final = false } = {}) {
    ...
    return [
      ...
      {
        entity_id: entity('binary_sensor', 'bus_power'),
        // From the gateway's own report of each line. Unknown until it has
        // reported one, which is not the same as powered.
        state: final || !gw?.lines ? 'unavailable'
          : Object.values(gw.lines).every((l) => l.status === 'ok') ? 'on' : 'off',
        attributes: {
          friendly_name: 'DALI bus power',
          device_class: 'power',
          lines: gw?.lines ?? null,
        },
      },
  ...
  function extraStates({ final = false } = {}) {
    const out = [];
    for (const provide of extras) {
      try {
        for (const s of provide() ?? []) {
          if (s && typeof s.entity_id === 'string') out.push(final ? { ...s, state: 'unavailable' } : s);
        }
      } catch {
        // One broken provider must not stop the bridge's own sensors.
      }
    }
    return out;
  }
  ...
  async function round(opts) {
    stats.rounds += 1;
    lastRoundAt = now();
    for (const s of [...states(opts), ...extraStates(opts)]) {
      let ok = false;
      try {
  ...
  return { noteEvent, publish, start, stop, states, touch: () => soon(), stats: () => ({ ...stats }) };
}
