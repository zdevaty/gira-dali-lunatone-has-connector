...
export function createUiServer({
  ...
  census = null,
  devices = null,
  // Gateway device list and scans. `gatewayWrites` false leaves the list
  // readable and every write refused -- the app option that turns this off.
  gateway = null,
  gatewayWrites = false,
  // The Gateway page and the device extras: { reader, config, snapshots,
  // watch, telemetry }. See gateway-routes.js.
  gatewayServices = null,
  // The Tuning page. `tuningApplied` false: saved, but control is off.
  tuning = null,
  tuningApplied = false,
  ...
  const gatewayRoutes = createGatewayRoutes({
    admin: gateway, liveness, ha, devices, writes: gatewayWrites, ...(gatewayServices ?? {}),
  });
  ...
  async function route(req, res) {
    ...
    if (req.method === 'GET' || req.method === 'HEAD') {
      switch (p) {
        ...
        case '/api/gateway/devices':
          if (!gateway) return json(res, 503, { error: 'gateway administration is not available' });
          return json(res, 200, { ...(await gateway.listDevices()), writes_enabled: gatewayWrites });
        case '/api/gateway/scan':
          if (!gateway) return json(res, 503, { error: 'gateway administration is not available' });
          return json(res, 200, { ...gateway.status(), writes_enabled: gatewayWrites });
        case '/api/tuning':
          if (!tuning) return json(res, 503, { error: 'tuning is not available' });
          return json(res, 200, { ...tuning.snapshot(), applied: tuningApplied });
        case '/api/entities':
          return json(res, 200, { lights: ha ? await ha.listLights() : [], reachable: ha ? !ha.isDown() : null });
        default: {
          if (p.startsWith('/api/gateway/')) {
            const r = await gatewayRoutes.get(p, url);
            if (r) return json(res, r.code, r.body);
          }
    ...
    if (p === '/api/tuning' || p.startsWith('/api/tuning/')) {
      if (!tuning) return json(res, 503, { error: 'tuning is not available' });
      let body = {};
      try {
      ...
      let r = null;
      if (req.method === 'PUT' && p === '/api/tuning') r = await tuning.save(body);
      else if (req.method === 'POST' && p === '/api/tuning/undo') r = await tuning.undo();
      else if (req.method === 'POST' && p === '/api/tuning/reset') r = await tuning.reset(body?.keys ?? null);
      if (!r) return json(res, 404, { error: 'no such endpoint' });
      return json(res, r.ok ? 200 : r.code, { ...r, applied: tuningApplied });
    }

    if (p.startsWith('/api/gateway/')) return gatewayWrite(req, res, p);

    return json(res, 404, { error: 'no such endpoint' });
  }

  // Everything here reaches the bus through the gateway, so it answers only a
  // person pressing a button: the guard header has already been checked, and
  // the option can switch the lot off.
  async function gatewayWrite(req, res, p) {
    let body = {};
    try {
      const text = await readBody(req, 16 * 1024);
      body = text ? JSON.parse(text) : {};
    ...
    const extra = await gatewayRoutes.write(req.method, p, body);
    if (extra) return json(res, extra.code, extra.body);

    if (!gateway) return json(res, 503, { error: 'gateway administration is not available' });
    if (!gatewayWrites) {
      return json(res, 403, { error: 'device management is switched off in the app configuration' });
    }

    if (req.method === 'POST' && p === '/api/gateway/scan') {
      const r = await gateway.startScan(body?.mode);
      return json(res, r.ok ? 200 : r.code, r);
    }
    if (req.method === 'POST' && p === '/api/gateway/scan/cancel') {
      const r = await gateway.cancelScan();
      return json(res, r.ok ? 200 : r.code, r);
    }
    const m = /^\/api\/gateway\/device\/(\d+)$/.exec(p);
    if (req.method === 'PUT' && m) {
      const r = await gateway.updateDevice(Number(m[1]), body);
      return json(res, r.ok ? 200 : r.code, r);
    }
    return json(res, 404, { error: 'no such endpoint' });
  }
