...
for (const tab of document.querySelectorAll('.tab')) {
  tab.addEventListener('click', () => {
    ...
    if (tab.dataset.panel === 'hub') loadHub();
    if (tab.dataset.panel === 'tuning') openTuning(); else closeTuning();
  });
}
...
function describe(e) {
  switch (e.kind) {
    ...
    case 'command':
      if (e.scope === 'special') return `special ${e.command}`;
      if (e.scope === 'gear') return e.command.replace(/_/g, ' ');
      return `cmd inst=${e.instance} op=${e.opcode}`;
    case 'response': {
      const who = e.to?.target ? `${e.to.target.replace('short', 'A')} ` : '';
      if (e.status) {
        const set = Object.entries(e.status).filter(([, on]) => on).map(([n]) => n.replace(/_/g, ' '));
        return `reply ${who}status: ${set.length ? set.join(', ') : 'ok, lamp off'}`;
      }
      if (e.to?.query === 'query_actual_level') return `reply ${who}level ${e.level ?? 'unknown (MASK)'}`;
      return `reply ${e.value}`;
    }
    ...
    // Settings changes put nothing on the bus; the reads query it but change nothing.
    case 'gateway_write': {
      const what = e.effect === 'config' ? 'gateway setting'
        : /_read$/.test(e.action ?? '') ? 'bus read' : 'bus write';
      return `${what}: ${e.action}${fields({ ...e, action: undefined, effect: undefined })}`;
    }
    case 'ha_reload': return `HA ${e.domain} reload ${e.ok ? 'ok' : 'FAILED'}${e.method ? ` (${e.method})` : ''}`;
    case 'tuning': return e.changed
      ? `tuning ${e.action}: ${Object.entries(e.changed).map(([k, v]) => `${k} ${JSON.stringify(v.from)} → ${JSON.stringify(v.to)}`).join(', ')}`
      : `tuning ${e.action}`;
    case 'gateway_snapshot': return `gateway setup copy (${e.reason}): ${e.changed ? `${e.changes ?? 'first copy'}${e.changes ? ' change(s)' : ''}` : 'no changes'}${e.first?.length ? ` — ${e.first.join('; ')}` : ''}`;
    case 'gateway_automation': return `gateway ${e.automation} “${e.name}” due — ${e.summary ?? ''}${e.targets?.length ? ` → ${e.targets.join(', ')}` : ''} (not sent by this app)`;
...
function statusBadges(dev) {
  ...
  const reading = gwReadings.get(dev.id);
  if (reading?.summary?.failures?.length) wrap.append(el('span', 'badge bad', 'driver reports a problem'));
  return wrap;
}
