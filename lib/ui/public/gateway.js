...
function renderClock(o) {
  ...
  if (!c) {
    ...
    const drift = c.drift_s == null ? null
      : Math.abs(c.drift_s) < 5 ? 'right' : `${Math.abs(c.drift_s) >= 120 ? `${Math.round(Math.abs(c.drift_s) / 60)} min` : `${Math.abs(c.drift_s)} s`} ${c.drift_s > 0 ? 'fast' : 'slow'}`;
    host.append(kv([
