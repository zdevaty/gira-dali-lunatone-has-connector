...
export function createHaClient({ url, token, log = () => {}, timeoutMs = 5000, fetchImpl = fetch } = {}) {
  ...
  // Every light Home Assistant knows about, for the commissioning page's picker.
  // Read-only, and it degrades to an empty list rather than throwing: an
  // unreachable HA must not break the page that would tell you HA is unreachable.
    ...
    try {
      ...
      return states
        ...
        .map((s) => ({
          ...
          state: s.state,
          supports_color_temp: Array.isArray(s.attributes?.supported_color_modes)
            && s.attributes.supported_color_modes.includes('color_temp'),
          min_kelvin: s.attributes?.min_color_temp_kelvin ?? null,
          max_kelvin: s.attributes?.max_color_temp_kelvin ?? null,
        }))
  ...
  // Returns whether Home Assistant actually took it. Callers use that to slow
  // down: hammering a service that is timing out makes it slower still.
  async function callService(domain, service, body) {
    const res = await request(`/api/services/${domain}/${service}`, {
      method: 'POST',
  ...
  // Writes a state object into Home Assistant's state machine -- the status
  // sensors. Not a registry entity: HA forgets it on restart until the next
  // write, which is why the sensors are republished on a heartbeat.
  ...
  // Home Assistant's own settings: time zone and location, for the gateway's
  // clock and sunrise schedules. Null when unavailable.
  async function getConfig() {
    ...
    try {
      ...
      return {
        time_zone: typeof cfg?.time_zone === 'string' ? cfg.time_zone : null,
        latitude: typeof cfg?.latitude === 'number' ? cfg.latitude : null,
        longitude: typeof cfg?.longitude === 'number' ? cfg.longitude : null,
      };
    } catch {
      return null;
    }
  ...
  // Renders a template. The REST API reaches the area and device registries
  // no other way. A refusal is an answer, not an outage, so like the reload
  // below this does not mark Home Assistant as down.
  async function renderTemplate(template) {
    try {
      const res = await fetchImpl(`${baseUrl}/api/template`, {
        method: 'POST',
      ...
      if (!res.ok) return { ok: false, status: res.status, error: `HTTP ${res.status}${text ? `: ${text.slice(0, 200)}` : ''}` };
      return { ok: true, text };
    } catch (err) {
  ...
  // Reloads an integration so it picks up devices added since it loaded --
  // after a gateway scan, the Lunatone integration has not heard of the new
  // light until this happens.
  //
  // Two ways, because neither is certain everywhere. The config-entries HTTP
  // view reloads exactly the integration asked for, but it is the frontend's
  // API, not the documented REST one, and a scoped token may not reach it. The
  // documented fallback is homeassistant.reload_config_entry aimed at an entity
  // we already know belongs to it. Neither path marks HA as down on a refusal:
  // "not allowed" is an answer, not an outage.
  async function reloadIntegration(domain, { viaEntities = [] } = {}) {
    const headers = { Authorization: `Bearer ${token}`, 'Content-Type': 'application/json' };
    const attempts = [];
    try {
      const res = await fetchImpl(`${baseUrl}/api/config/config_entries/entry?domain=${encodeURIComponent(domain)}`, {
        headers, signal: AbortSignal.timeout(timeoutMs),
      });
      const entries = res.ok ? await res.json().catch(() => null) : null;
      const ids = Array.isArray(entries)
        ? entries.filter((e) => e?.domain === domain && typeof e.entry_id === 'string').map((e) => e.entry_id)
        : [];
      attempts.push({ method: 'config_entries', status: res.status, entries: ids.length });
      if (ids.length) {
        let reloaded = 0;
        for (const id of ids) {
          const r = await fetchImpl(`${baseUrl}/api/config/config_entries/entry/${encodeURIComponent(id)}/reload`, {
            method: 'POST', headers, signal: AbortSignal.timeout(timeoutMs * 4),
          });
          if (r.ok) reloaded += 1;
        }
        if (reloaded === ids.length) return { ok: true, method: 'config_entries', entries: reloaded, attempts };
        attempts[attempts.length - 1].reloaded = reloaded;
      }
    } catch (err) {
      attempts.push({ method: 'config_entries', error: String(err?.message ?? err) });
    }

    const entity = viaEntities.find((e) => typeof e === 'string' && e);
    if (entity) {
      const ok = await callService('homeassistant', 'reload_config_entry', { entity_id: entity });
      attempts.push({ method: 'service', entity, ok });
      if (ok) return { ok: true, method: 'service', entity, attempts };
    }
  ...
  async function preflight(deviceMap = {}) {
    ...
    // The base URL, not its origin. Under the Supervisor the API lives behind a
    // path -- http://supervisor/core/api -- and using the origin alone would ask
    // http://supervisor/api and report a confident, wrong diagnosis.
    const apiRoot = baseUrl;
    log({ kind: 'preflight', step: 'config', url: apiRoot, token: token ? `present (${String(token).length} chars)` : 'MISSING' });
    ...
    try {
      const res = await fetchImpl(`${apiRoot}/api/`, {
        headers: { Authorization: `Bearer ${token}`, 'Content-Type': 'application/json' },
        signal: AbortSignal.timeout(timeoutMs),
      });
      if (res.status === 401) {
      ...
      if (res.status === 404) {
        fail('api', `reached ${apiRoot} but /api/ returned 404`, 'Something is listening there, but it is not the HA API. Check the port — HA behind a proxy is usually :80, direct is :8123.');
        return false;
      }
      if (!res.ok) {
        fail('api', `GET ${apiRoot}/api/ returned HTTP ${res.status}`);
        return false;
      }
    ...
    for (const [address, mapping] of Object.entries(deviceMap)) {
      ...
      try {
        const res = await fetchImpl(`${apiRoot}/api/states/${encodeURIComponent(entity)}`, {
  ...
  return { getLightKelvin, getLightBrightness, getLightState, listLights, callService, setState, getConfig, renderTemplate, reloadIntegration, preflight, isDown: () => down };
}
