...
const DATE_SHAPES = [
  // What firmware 1.18.7/1.4.6 actually returns, seen on the Pi on 16 Sep
  // 2026: "14. February 2019". English month names only; the day was two
  // digits there, so whether a single-digit day is padded is not known, and
  // it is written unpadded.
  {
    name: 'dayMonthName',
    re: /^(\d{1,2})\.\s+([A-Za-z]+)\s+(\d{4})$/,
    pick: (m) => {
      const month = MONTH_NAMES.findIndex((n) => n.toLowerCase() === m[2].toLowerCase()) + 1;
      return [m[3], month || 'invalid', m[1]];
    },
    format: (y, mo, d) => `${Number(d)}. ${MONTH_NAMES[Number(mo) - 1]} ${Number(y)}`,
  },
...
export function parseGatewayDateTime(date, time) {
  ...
  if (!Number.isInteger(month) || month < 1 || month > 12 || day < 1 || day > 31 || hour > 23 || minute > 59 || second > 59) return null;
