...
export function createGatewayAdmin({
  ...
  let stopping = false;
  let identifying = null; // the running identify, so stop() can wait for its restore
  ...
  async function identify(id) {
    ...
    if (stopping) return { ok: false, code: 503, error: 'the app is stopping' };
    const run = exclusive('identify', { id }, async () => {
    ...
    try {
      ...
      if (identifying === run) identifying = null;
    }
  }
  ...
  // Stops polling, and cuts a blink short -- resolving once its light is back.
  function stop() {
    stopping = true;
    if (pollTimer) clearTimer(pollTimer);
    pollTimer = null;
    return identifying ? identifying.then(() => {}, () => {}) : Promise.resolve();
  }
