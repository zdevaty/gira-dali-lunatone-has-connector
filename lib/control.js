...
export function createController({
  ...
  function noteCommand(st) {
    ...
    if (pending && pending.address !== st.address && now() - pending.at <= correlationWindowMs) {
  ...
  function correlate(gear) {
    ...
    if (now() - pending.at > correlationWindowMs) return;
    ...
    if (entry.gear && entry.gear !== topGear && !reported.has(address)) {
      ...
      log({ kind: 'alert', alert: 'gear_mapping_learned', target: `short${address}`, entity: entry.entity,
        ...
        note: 'measured by correlating this daemon\'s own HA calls with arc levels; add it to devices.json to make it explicit' });
    }
  }
  ...
  function stateFor(address) {
    ...
    if (!st) {
      st = {
        ...
        // the order the knob produced them.
  ...
  function isColourGesture(st) {
    return st.held || now() < st.graceUntil;
  }
  ...
  function scheduleFlush(st, mapping) {
    const t = now();
    if (t - st.lastFlush >= flushMs) {
      flush(st, mapping);
    } else if (!st.timer) {
      st.timer = setTimer(() => {
        st.timer = null;
        flush(st, mapping);
      }, flushMs - (t - st.lastFlush));
    }
  }
  ...
  function send(st, logEvent, call) {
    log(logEvent);
    st.chain = st.chain.then(call).catch(() => {});
  }
    ...
    st.chain = st.chain
      .then(async () => {
        ...
        if (current == null) {
          ...
          noteCommand(st);
          await ha.callService('light', 'turn_on', { entity_id: mapping.entity, brightness: minBrightness });
          return;
          ...
          noteCommand(st);
          await ha.callService('light', 'turn_on', { entity_id: mapping.entity, brightness: 255 });
          return;
        }
        ...
        noteCommand(st);
        await ha.callService('light', 'turn_on', { entity_id: mapping.entity, brightness_step: step });
      })
      .catch(() => {});
  }
  ...
  function sendColour(st, mapping, delta) {
    ...
    st.chain = st.chain
      .then(async () => {
        ...
        noteCommand(st);
        await ha.callService('light', 'turn_on', { entity_id: mapping.entity, color_temp_kelvin: kelvin });
      })
      .catch(() => {});
  }
  ...
  function settled() {
    return Promise.all([...devices.values()].map((st) => st.chain));
  }

  return { handleEvent, observeLevel, settled };
}
