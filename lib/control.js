...
export function createController({
  ...
  // What a knob feels like, applied live from the Tuning page. Read at use
  // time everywhere below, so a change takes effect at the next report without
  // touching any gesture in progress -- which is the whole point: turn, watch,
  // adjust, turn again, without a restart costing the wall switches.
  const tuning = { flushMs, speedCurve: [...speedCurve], rampEveryReports, levelDivergence, maxQueue, staleMs, brightnessGain, colourGain, minBrightness };

  // Callers validate (lib/tuning.js); this only refuses keys it does not own,
  // so a typo cannot quietly add a setting nothing reads.
  function setTuning(next = {}) {
    for (const [key, value] of Object.entries(next)) {
      if (!Object.hasOwn(tuning, key)) throw new Error(`unknown tuning key ${key}`);
      tuning[key] = Array.isArray(value) ? [...value] : value;
    }
    return getTuning();
  }
  const getTuning = () => ({ ...tuning, speedCurve: [...tuning.speedCurve] });
  ...
  let haBackoffMs = 0;
  const effectiveFlushMs = () => Math.max(tuning.flushMs, haBackoffMs);
  ...
  function noteCallResult(ok) {
    if (ok !== false) {
      ...
      haBackoffMs = haBackoffMs <= tuning.flushMs ? 0 : Math.floor(haBackoffMs / 2);
      return;
    }
    const next = haBackoffMs === 0 ? tuning.flushMs * 2 : haBackoffMs * 2;
  ...
  function noteCommand(st) {
    // A real command is going out, so whatever we last declined to send is
    // history: the next time we decline, say so again. Cleared here rather than
    // on enqueue, because an attempt that ends in suppression enqueues too.
    st.suppressedFor = null;
  ...
  function onPosition(st, mapping, event) {
    ...
    if (raw !== 0) {
      ...
      st.tierSign = sign;
      st.pinnedReports = 0;
      delta = sign * tuning.speedCurve[idx];
    } else {
      ...
      const climbed = Math.floor((st.pinnedReports - 1) / tuning.rampEveryReports);
      const idx = Math.min(tuning.speedCurve.length - 1, (st.tierIdx ?? 0) + climbed);
      delta = sign * tuning.speedCurve[idx];
    }
  ...
  // Says why nothing was sent -- but once per run, not once per flush. Holding a
  // knob against a stop produces ten reports a second, and ten identical "no
  // change" lines a second is how you lose the line that mattered. Cleared
  // whenever a real command goes out, so the next stop is reported afresh.
  function suppressed(st, mapping, reason, action = 'brightness_suppressed') {
    if (st.suppressedFor === reason) return;
    st.suppressedFor = reason;
    log({ kind: 'control', action, target: st.target, entity: mapping.entity, reason });
  }
  ...
  function enqueue(st, label, fn) {
    ...
    while (st.queue.length > tuning.maxQueue) {
      st.queue.shift();
      st.dropped += 1;
  ...
  function flush(st, mapping) {
    ...
    if (st.pendingBrightness !== 0) {
      const step = Math.round(st.pendingBrightness * tuning.brightnessGain);
  ...
  function sendBrightness(st, mapping, delta) {
    enqueue(st, 'brightness', async () => {
      ...
      if (
        ...
        Math.abs(st.brightness - st.observedLevel) > tuning.levelDivergence
      ) {
        const target = Math.min(255, Math.max(tuning.minBrightness, st.observedLevel + delta));
        log({
      ...
      if (current == null) {
      ...
      } else if (current + delta <= tuning.minBrightness) {
        ...
        if (current <= tuning.minBrightness) return suppressed(st, mapping, 'resting on the floor');
        st.brightness = tuning.minBrightness;
        log({ kind: 'control', action: 'brightness', target: st.target, entity: mapping.entity, brightness: tuning.minBrightness, floored: true });
        noteCommand(st);
        noteCallResult(await ha.callService('light', 'turn_on', { entity_id: mapping.entity, brightness: tuning.minBrightness }));
        return;
  ...
  function sendColour(st, mapping, delta) {
    ...
    enqueue(st, 'colour', async () => {
      ...
      const before = Math.round(st.colourKelvin);
      const stepKelvin = (delta * (max - min) * tuning.colourGain) / 255;
      st.colourKelvin = Math.min(max, Math.max(min, st.colourKelvin + stepKelvin));
      const kelvin = Math.round(st.colourKelvin);

      // Hard against an end of the range and still being pushed outwards. The
      // clamp gives back the value we already sent, so sending it again buys
      // nothing and costs a full DT8 sequence on the bus and a Home Assistant
      // call -- ten times a second while the knob is held there. Measured on the
      // real installation: this is what filled the queue and dropped commands.
      //
      // Brightness has had this at both ends from the start; colour never did.
      if (kelvin === before && (st.colourKelvin === min || st.colourKelvin === max)) {
        return suppressed(
          st, mapping,
          st.colourKelvin === min ? 'colour already at the warm end' : 'colour already at the cool end',
          'colour_suppressed',
        );
      }
  ...
  return { handleEvent, observeLevel, settled, setDeviceMap, setTuning, getTuning };
}
