...
// 16-bit questions to control gear, by name. Only these two, because they are
// the ones seen on this bus (the gateway polls every driver with them since the
// scan of 16 Sep 2026: `01 90`, `01 A0`, `03 90`, ...) and both are the
// IEC 62386-102 standard opcodes. Every other gear command stays `raw` with its
// bytes: a name that is merely plausible is worse than none.
const GEAR_QUERIES = {
  0x90: 'query_status',
  0xa0: 'query_actual_level',
};

// The answer to query_status, bit 0 first. Same order, and the same meaning, as
// the gateway's own DeviceStatusModel -- which is what its "lamp failure" badge
// is built from.
const STATUS_BITS = [
  'control_gear_failure',
  'lamp_failure',
  'lamp_on',
  'limit_error',
  'fade_running',
...
function decodeStatus(value) {
  return Object.fromEntries(STATUS_BITS.map((name, bit) => [name, Boolean(value & (1 << bit))]));
}
...
export function createDecoder() {
  ...
  function decode16(bytes, tsMs) {
    ...
    if (b0 === 0xa1 && b1 === 0x00) {
      // TERMINATE: the end of an addressing session. This was decoded as an alert
      // called dali_reset until 16 Sep 2026, a name taken on assumption. It is
      // the IEC 62386-102 special command 0xA1 0x00, it is what every scan and
      // every DALI Cockpit session sends when it is done (all five in the
      // 25 Aug capture came in the middle of Cockpit work), and it resets
      // nothing. RESET is an addressed command, not this.
      pendingQuery = null;
      return { kind: 'command', scope: 'special', command: 'terminate', bits: 16, bytes: bytesStr };
    }
    ...
    const query = GEAR_QUERIES[b1];
    if (query) {
      // Only a short address has exactly one answerer. A group or broadcast
      // query gets overlapping answers, which is a collision, not a value.
      const address = target.type === 'short' ? b0 >> 1 : null;
      if (address != null) pendingQuery = { space: 'gear', target: target.label, address, query, opcode: hexByte(b1), tsMs };
      return { kind: 'command', scope: 'gear', target: target.label, command: query, opcode: hexByte(b1), bits: 16, bytes: bytesStr };
    }
  ...
  function decode8(bytes, tsMs) {
    ...
    if (pendingQuery && tsMs - pendingQuery.tsMs <= RESPONSE_WINDOW_MS) {
      ...
      pendingQuery = null;
      const event = { kind: 'response', to, value, bits: 8, bytes: bytesStr };
      if (to.query === 'query_status') event.status = decodeStatus(value);
      // 255 is MASK: the gear cannot say, which is what it answers with a failed
      // lamp. Not a level of 255.
      if (to.query === 'query_actual_level') event.level = value === 0xff ? null : value;
      return event;
    }
  ...
  function decodeFrame(bits, bytes, tsMs = Date.now()) {
    ...
    // An answer can only belong to the forward frame directly before it. Any
    // other forward frame in between -- a level, a register write, a command
    // we do not name -- means whatever was asked earlier went unanswered, and a
    // later answer must not be credited to it. Queries re-arm below.
    pendingQuery = null;
    if (bits === 16) return decode16(bytes, tsMs);
